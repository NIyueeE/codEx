...
- 每个主线程用户回合提交前,TUI 将工作区快照到
  ...
  相对路径、大小、mtime,外加 prompt 摘要)以及 `files/` 下的文件内容。
- 快照是**增量**的:只有自上次快照以来大小或 mtime 发生变化的文件才会被
  重新复制,因此连续多轮的开销很小。
- 遍历会跳过 `.git`,默认遵循 `.gitignore` 文件。
- 每个工作区只保留最近 **20 个**快照;更旧的会被清理。
- 恢复时把快照文件复制回工作区,并删除快照之后创建的文件(被 gitignore 的
  路径保持不动)。恢复较旧快照时,去重的文件会沿增量链回溯找到内容;如果
  链上任何位置都不存在该文件的副本,恢复会失败并列出缺失路径,而不是静默
  保留当前内容。
